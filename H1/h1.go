...
func h1_im() (save, erro, executeAll bool) {
	...
	if len(clusters) > 0 {
		for indice_cluster, cluster := range clusters {
			...
			fmt.Println(" Indice do Cluster: ", indice_cluster)
			fmt.Println(" Identificador do Cluster: ", identificador_cluster)
			for _, endereco := range map_enderecos {
				...
				if tamanho_clusters_encontrados > 0 {
					...
					CountCluster(identificador_cluster, identificadores, map_enderecos_resultante,
						clusters_que_contem_o_endereco, ConnectionMongo, DB_Cluster, Collection_Cluster_Map)
...
func CountCluster(identificador_cluster string, identificadores []string, mapaAtual map[string]string, clusters_que_contem_o_endereco []Model.MapCluster, ConnectionMongo, DB_Cluster, Collection_Cluster_Map string) {
	identificadoresSeraoBuscados := append([]string{identificador_cluster}, identificadores...)
	AllClusterIdentificadorCluster := Function.SearchAddrMapsClusters(100000000, identificadoresSeraoBuscados, ConnectionMongo, DB_Cluster, Collection_Cluster_Map)

	clusterResultante := append(AllClusterIdentificadorCluster, clusters_que_contem_o_endereco...)
...
func ProcessCluster_M1(tamanho_map_enderecos_resultante int, identificadores []string, identificadorBase, ConnectionMongo, DB_Cluster,
	Collection_Cluster_Map, Collection_Identificadores string) bool {
	var confirm bool
	...
	if len(tamanho_identificador_base.Identificador) == 0 || tamanho_identificador_base.TamanhoCluster == 0 {
		fmt.Println(" Erro busca pelo identificador base")
	}
